...
class ChatViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate{
	...
	func textFieldDidBeginEditing(_ textField: UITextField) {
		...
		UIView.animate(withDuration: 0.5) {
			self.heightConstraint.constant = 308 // keyboard is 258px (258+50)
    ...
    @IBAction func sendPressed(_ sender: AnyObject) {
		...
		messagesDatabase.childByAutoId().setValue(messageDictionary) {
			...
			if error != nil {
				...
				print("Messages saved to Firebase ✅")
	...
	func receiveMessages() {
		...
		messagesDB.observe(.childAdded) { (snapshot) in
			...
			guard let text = snapshotValue["MessageBody"], let sender = snapshotValue["Sender"] else { return }
			let message = Message()
